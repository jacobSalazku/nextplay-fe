    ...
    >
      {phases.map((phase, index) => (
        ...
        >
          <button
            ...
            className={cn(
              'block w-14 touch-none overflow-hidden rounded-md border bg-slate-800',
              index === activeIndex
          ...
          >
            ...
            />
            <span className="block bg-slate-900/80 py-0.5 text-center text-[10px] font-semibold">
              {index + 1}
            </span>
          </button>
          ...
          {phases.length > 1 && (
            <button
              ...
              className="absolute -top-1.5 -right-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-slate-700 text-gray-300 hover:bg-red-600 hover:text-white"
            >
      ...
      {phases.length < MAX_PHASES && (
        <button
          ...
          onClick={onAdd}
          className="flex h-[3.25rem] w-14 shrink-0 items-center justify-center rounded-md border border-dashed border-white/15 text-gray-500 hover:border-white/30 hover:text-gray-300"
        >
